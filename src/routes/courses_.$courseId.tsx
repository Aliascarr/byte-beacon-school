...
export function CourseViewerPage({ courseIdOverride, onClose }: { courseIdOverride?: string; onClose?: () => void }) {
  ...
  if (!course) {
    return (
      <div className="mx-auto w-[min(1180px,92vw)] py-24 text-center">
        ...
        {onClose ? (
          <Button onClick={onClose} className="mt-6">Вернуться к списку курсов</Button>
        ) : (
    ...
    <div className="mx-auto w-[min(1280px,94vw)] pb-24 pt-6">
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        {onClose ? (
          <button
            onClick={onClose}
            className="inline-flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-foreground transition-colors glass rounded-full px-4 py-2"
          >
            <ArrowLeft className="size-4" /> Назад к списку курсов
          </button>
        ) : (
          <Link
            to="/courses"
      ...
      <div className="grid gap-6 lg:grid-cols-12">
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="glass-card p-5 rounded-2xl space-y-4">
            ...
            <div className="space-y-4">
              {course.modules?.map((mod: LessonModule) => (
